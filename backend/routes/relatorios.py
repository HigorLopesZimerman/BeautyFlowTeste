...
def relatorios(usuario_id):
    ...
    condicao_agendamento = f" WHERE a.usuario_id = {usuario_id} "
    condicao_pagamento = f" WHERE p.usuario_id = {usuario_id} "
    params_agendamento = []
    params_pagamento = []
    
    if inicio and fim:
        condicao_agendamento += " AND a.data BETWEEN ? AND ? "
        condicao_pagamento += " AND p.data_pagamento BETWEEN ? AND ? "
        params_agendamento.extend([inicio, fim])
        params_pagamento.extend([inicio, fim])
    ...
    def query_agendamento(base_query, extra_cond=""):
        ...
        if extra_cond:
            where_clause += " AND " + extra_cond
        return conexao.execute(base_query + where_clause, params_agendamento).fetchone()
    ...
    def query_pagamento(base_query, extra_cond=""):
        ...
        if extra_cond:
            where_clause += " AND " + extra_cond
        return conexao.execute(base_query + where_clause, params_pagamento).fetchone()
    ...
    pagamentos_pendentes = query_pagamento("SELECT COUNT(*) FROM pagamentos p", "LOWER(p.status) = 'pendente'")[0]
    
    valor_pendente_row = query_pagamento("SELECT SUM(p.valor) FROM pagamentos p", "LOWER(p.status) = 'pendente'")
    valor_pendente = valor_pendente_row[0] if valor_pendente_row[0] else 0

    faturamento_row = query_pagamento("SELECT SUM(p.valor) FROM pagamentos p", "LOWER(p.status) = 'pago'")
